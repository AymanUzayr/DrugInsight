...
from gnn_encoder_crossattn   import GNNEncoderCrossAttn
from cross_attn_classifier   import CrossAttnDDIClassifier
...
# ── Feature extractor for negative sampling ───────────────────────────────────
...
# Note: smaller batch size due to variable atom counts in cross-attention
...
# ── Models ─────────────────────────────────────────────────────────────────────
gnn        = GNNEncoderCrossAttn().to(DEVICE)
classifier = CrossAttnDDIClassifier(extra_features=6).to(DEVICE)

# Separate learning rates — GNN slower to prevent memorisation
...
with torch.no_grad():
    ...
    if sample_batch:
        ...
        g_b = g_b.to(DEVICE)
        atom_embeds = gnn(g_a)
        print(f"Atom embedding shape: {atom_embeds.shape}")   # [N_total, 256]
        print(f"Embedding mean: {atom_embeds.mean().item():.4f}")
        print(f"Embedding std:  {atom_embeds.std().item():.4f}")
        print(f"Any NaN:        {torch.isnan(atom_embeds).any()}")
...
def train_epoch(loader):
    ...
    for batch in loader:
        ...
        atom_a = gnn(g_a)   # [N_total_a, 256]
        atom_b = gnn(g_b)   # [N_total_b, 256]

        # Symmetry augmentation
        if torch.rand(1).item() > 0.5:
            atom_a, atom_b = atom_b, atom_a
            g_a.batch, g_b.batch = g_b.batch, g_a.batch

        prob, _ = classifier(atom_a, g_a.batch, atom_b, g_b.batch, extra)
        prob = prob.view(-1)
